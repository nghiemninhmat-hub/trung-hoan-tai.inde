...
export default function MapPage() {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-8">
      <header className="relative overflow-hidden rounded-3xl border border-[#670201]/30 bg-gradient-to-br from-[#1c0908] via-[#110707] to-[#080405] px-6 py-10 text-center sm:px-12">
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-serif text-4xl font-bold tracking-wide text-amber-100 sm:text-6xl">Địa Lý Trùng Hoan</h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-gray-400 sm:text-base">Bản đồ các vùng đất, kinh đô, sơn mạch và hải vực nơi những linh hồn tìm đường trở về hiện thực.</p>
          <div className="mx-auto mt-6 flex items-center justify-center gap-3 text-[10px] uppercase tracking-[0.3em] text-[#c68b62]/70">
            <span className="h-px w-12 bg-[#670201]/60" />
            <span>Trung Hoan Tái</span>
            <span className="h-px w-12 bg-[#670201]/60" />
          </div>
        </div>
      </header>
      ...
      <section className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="relative overflow-hidden rounded-3xl border border-[#670201]/25 bg-[#0a0505] p-2 shadow-[0_20px_80px_rgba(0,0,0,0.35)] sm:p-4">
          <div className="relative aspect-[16/10] min-h-[440px] overflow-hidden rounded-2xl border border-[#c68b62]/20 bg-[#261614]">
            <img src="/trung-hoan-map.webp" alt="Địa đồ minh họa các vùng đất Trùng Hoan" className="absolute inset-0 h-full w-full object-cover opacity-80 mix-blend-screen" />
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_30%,rgba(8,3,4,0.62)_100%)]" />
            <div className="absolute inset-0 bg-[#2d0b08]/20 mix-blend-color" />

            <div className="absolute left-5 top-5 rounded-lg border border-amber-300/30 bg-black/45 px-3 py-2 text-[10px] uppercase tracking-[0.22em] text-amber-200/80 backdrop-blur-sm">
              Vương triều Trùng Hoan
            </div>
            <div className="absolute bottom-5 left-5 flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-gray-300/60">
              <span className="h-px w-8 bg-amber-200/50" />
              Bản đồ lưu trữ — không theo tỷ lệ
            </div>
            ...
            {places.map(place => {
              ...
              return (
                <button key={place.name} onClick={() => setSelectedPlace(place)} className={`group absolute -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ${place.position} ${active ? 'z-20 scale-110' : 'z-10 hover:z-20 hover:scale-105'}`}>
                  <span className={`flex items-center gap-1.5 rounded-full border px-2 py-1 text-[10px] font-semibold shadow-lg backdrop-blur-md transition-colors sm:px-2.5 sm:text-xs ${active ? 'border-amber-200/70 bg-[#670201]/80 text-amber-100 shadow-[#670201]/40' : 'border-white/25 bg-black/60 text-gray-200 group-hover:border-amber-300/60 group-hover:text-amber-100'}`}>
                    <Icon className="h-3 w-3" />
                    {place.name}
                  </span>
                  <span className={`mx-auto mt-1 block h-2 w-2 rounded-full border border-amber-100/70 bg-[#a00404] shadow-[0_0_12px_rgba(160,4,4,0.85)] ${active ? 'animate-pulse' : ''}`} />
                </button>
              );
            })}
          </div>
        ...
        <aside className="flex flex-col rounded-3xl border border-[#670201]/25 bg-gradient-to-b from-[#160908] to-[#0b0505] p-5 sm:p-6">
